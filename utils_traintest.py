...
def NT_Xent_sup(features_batch, temperature):
    ...
    pos_mask = torch.zeros_like(self_mask, dtype=torch.bool, device=cos_sim.device)
    for i in range(1, 4):
    ...
    sim_argsort = comb_sim.argsort(dim=-1, descending=True).cpu()
...
def train_contrastive_sup(model, device, train_loader, optimizer, lr_scheduler, epoch, loss_temperature):
    ...
    for _, batch in enumerate(train_loader):
        ...
        cat_imgs = torch.cat(batch_x, dim=0).to(device)

        #batch_x_resh = batch_x.reshape(-1, 1, 340, 100).to(device) dsnt work
        batch_x_split = torch.split(cat_imgs, 1, dim=1)
...
def prepare_data_features(model, dataset, device, supervised = False):
    """
    ...
    with torch.no_grad():
        ...
        if supervised: network.encoder.classification_layer = nn.Identity()
        else: network.classification_layer = nn.Identity() # MIGHT CHANGE THIS, OR PASS AS AN ARGUMENT!
        network.eval()
        network.to(device)
